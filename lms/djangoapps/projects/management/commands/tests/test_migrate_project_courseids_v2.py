...
class MigrateCourseIdsTests(TestCase):
    ...
    if settings.FEATURES.get('PROJECTS_APP', False):
        def test_migrate_project_courseids_v2(self):
            ...
            self.assertEqual(updated_submission_review.content_id, self.good_style_content_id)
            updated_submission_review = WorkgroupSubmissionReview.objects.get(id=workgroup_submission_review2.id)
            self.assertEqual(updated_submission_review.content_id, self.good_style_content_id2)
