...
if settings.FEATURES.get('ORGANIZATIONS_APP', False):
    urlpatterns += (
        url(r'^(?P<group_id>[0-9]+)/organizations/*$', groups_views.GroupsOrganizationsList.as_view(), name='groups-organizations-list'),
    )

if settings.FEATURES.get('PROJECTS_APP', False):
    urlpatterns += (
        url(r'^(?P<group_id>[0-9]+)/workgroups/*$', groups_views.GroupsWorkgroupsList.as_view(), name='groups-workgroups-list'),
    )
