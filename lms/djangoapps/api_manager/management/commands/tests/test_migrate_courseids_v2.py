...
class MigrateCourseIdsTests(TestCase):
    ...
    if settings.FEATURES.get('PROGRESS_APP', False):
        def test_migrate_courseids_v2(self):
            ...
            self.assertEqual(updated_completion.course_id, self.good_style_course_id)
            self.assertEqual(updated_completion.content_id, self.good_style_content_id)
            updated_completion = CourseModuleCompletion.objects.get(id=course_module_completion2.id)
            self.assertEqual(updated_completion.course_id, self.good_style_course_id2)
            self.assertEqual(updated_completion.content_id, self.good_style_content_id2)
            self.assertEqual(updated_completion.stage, self.good_style_content_id2)
